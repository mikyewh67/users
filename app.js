const state={items:[],platform:'all',kind:'all',sort:'price-asc',query:'',selected:null,paymentMethod:null,config:null,visible:48,promo:null};
...
const moneyExact=n=>new Intl.NumberFormat('en-US',{style:'currency',currency:'USD',minimumFractionDigits:2,maximumFractionDigits:2}).format(Number(n));
...
function currentAmount(){return state.promo?Number(state.promo.finalAmount):Number(state.selected?.sale_price||0)}
function renderCheckoutPrice(){if(!state.selected)return;const old=$('#checkoutOriginalPrice');if(state.promo){old.textContent=moneyExact(state.selected.sale_price);old.classList.add('show');$('#checkoutPrice').textContent=moneyExact(state.promo.finalAmount)}else{old.classList.remove('show');$('#checkoutPrice').textContent=moneyExact(state.selected.sale_price)}}
function openCheckout(id){
  state.selected=state.items.find(x=>Number(x.id)===Number(id));state.paymentMethod=null;state.promo=null;if(!state.selected)return;
  $('#checkoutHandle').textContent=state.selected.handle;$('#checkoutMeta').textContent=`${platformLabel(state.selected.platform)} · ${state.selected.kind}`;$('#checkoutStatus').className='status';$('#checkoutStatus').textContent='';$('#checkoutForm').reset();$('#submitOrder').style.display='block';$('#applyPromo').disabled=false;$('#applyPromo').textContent='Apply';$('#promoStatus').className='promo-status';$('#promoStatus').textContent='15% off for the first 10 customers, up to $25 off.';renderCheckoutPrice();renderPayments();$('#checkoutModal').showModal()
}
async function applyPromo(){
  if(!state.selected)return;const code=$('#promoCode').value.trim().toUpperCase();const btn=$('#applyPromo'),msg=$('#promoStatus');if(!code){msg.className='promo-status err';msg.textContent='Enter a promo code.';return}
  btn.disabled=true;btn.textContent='Applying…';msg.className='promo-status';msg.textContent='Checking launch availability…';
  try{
    const r=await fetch('/api/promo',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({inventoryId:state.selected.id,code})});const j=await r.json();if(!r.ok||!j.ok)throw new Error(j.error||'Could not apply promo');
    state.promo=j;$('#promoCode').value=j.code;renderCheckoutPrice();if(state.paymentMethod)showInstructions();const until=new Date(j.expiresAt).toLocaleTimeString([],{hour:'numeric',minute:'2-digit'});msg.className='promo-status ok';msg.textContent=`LAUNCH15 applied — you save ${moneyExact(j.discountAmount)}. Your launch slot is held until ${until}. ${j.remaining} promo spot${j.remaining===1?'':'s'} remain after yours.`;btn.textContent='Applied';
  }catch(e){state.promo=null;renderCheckoutPrice();if(state.paymentMethod)showInstructions();msg.className='promo-status err';msg.textContent=e.message;btn.disabled=false;btn.textContent='Apply'}
}
$('#applyPromo').onclick=applyPromo;$('#promoCode').addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();applyPromo()}});
...
function showInstructions(){const p=state.config?.payments?.[state.paymentMethod],amt=moneyExact(currentAmount());if(!p)return;let t='';if(state.paymentMethod==='cashapp')t=`Send exactly ${amt} to ${p.to}. Add ${state.selected.handle} in the note if possible.`;if(state.paymentMethod==='zelle')t=`Send exactly ${amt} by Zelle to ${p.to}. Recipient: ${p.name||'listed recipient'}.`;if(state.paymentMethod==='paypal')t=`Send exactly ${amt} to ${p.to}. Keep your PayPal transaction ID.`;if(state.paymentMethod==='crypto')t=`Send the USD-equivalent of ${amt} using ${p.network}: ${p.address}. Confirm the network before sending and paste the transaction hash below.`;$('#paymentInstructions').textContent=t}
$('#checkoutForm').addEventListener('submit',async e=>{
  e.preventDefault();if(!state.paymentMethod){setStatus('Choose a payment method first.',false);return}
  if(state.promo&&new Date(state.promo.expiresAt)<=new Date()){state.promo=null;renderCheckoutPrice();setStatus('Your LAUNCH15 hold expired. Apply the code again before paying/submitting.',false);$('#applyPromo').disabled=false;$('#applyPromo').textContent='Apply';return}
  const btn=$('#submitOrder');btn.disabled=true;btn.textContent='Submitting…';setStatus('Creating your order…',true);
  try{
    const payload={inventoryId:state.selected.id,email:$('#buyerEmail').value.trim(),paymentMethod:state.paymentMethod,payerName:$('#payerName').value.trim(),paymentRef:$('#paymentRef').value.trim(),note:$('#buyerNote').value.trim(),promoToken:state.promo?.promoToken||null,promoCode:state.promo?.code||null};
    const r=await fetch('/api/order',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(payload)});const j=await r.json();if(!r.ok||!j.ok)throw new Error(j.error||'Order could not be created');
    $('#checkoutStatus').className='status ok';$('#checkoutStatus').innerHTML=`<div><div>Order submitted${j.discountAmount>0?` with ${esc(j.discountCode)} applied`:''}. Save this order ID:</div><div class="order-id">${esc(j.orderId)}</div><p>Amount awaiting verification: ${moneyExact(j.finalAmount)}. Payment still requires manual verification.</p></div>`;btn.style.display='none';await refreshInventory()
  }catch(err){setStatus(err.message,false)}finally{btn.disabled=false;btn.textContent='Submit order for verification'}
});
